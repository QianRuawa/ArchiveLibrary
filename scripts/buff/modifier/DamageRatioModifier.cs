...
/// <summary>
/// 全局承伤率修正：正数降低受到伤害（Buff），负数增加受到伤害（Debuff）。
/// 注意："提升"才是减益（Amount 为负时）。
/// 每层修正 <see cref="MultiplierPerStack"/>%（默认 5%），与力量等增伤独立乘算。
/// 标题/描述/图标/标签颜色均由 <see cref="ModDynamicIconPower"/> 基类根据 Amount 符号自动处理。
/// </summary>
[RegisterPower]
public class DamageRatioModifier : ModDynamicIconPower
{
    public override PowerType Type => PowerType.Buff;
    public override PowerStackType StackType => PowerStackType.Counter;

    public override string IconId => "DamagedRatio";
    ...
    /// <summary>
    /// Amount > 0（Buff）→ 减少受到伤害，multiplier &lt; 1.0<br/>
    /// Amount &lt; 0（Debuff）→ 增加受到伤害，multiplier &gt; 1.0
    /// </summary>
